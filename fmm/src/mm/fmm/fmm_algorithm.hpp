...
struct FastMapMatchConfig{
   ...
   * @param  perturbation Indicating if there is perturbation for privacy applied on the GPS data
   */
  FastMapMatchConfig(int k_arg = 8, double r_arg = 300, double gps_error = 50,
    double reverse_tolerance = 0.0, bool perturbation=false);
...
class FastMapMatch {
 ...
 protected:
  ...
  void update_tg(TransitionGraph *tg,
                 ...
                 double reverse_tolerance = 0, bool perturbation=false);
  /**
  ...
  void update_layer(int level, TGLayer *la_ptr, TGLayer *lb_ptr,
                    ...
                    bool *connected, bool perturbation=false);
 private:
