...
def process_taxi(taxi_group):
    """Detect stops based only on spatial proximity."""
    stops, current_stop = [], []
    ...
    for _, row in taxi_group.iterrows():
        ...
        if len(current_stop) >= MIN_SAMPLES:  # Use MIN_SAMPLES instead of time duration
            stops.append({
...
def create_popup_content(cluster_group, amenities_gdf, spatial_idx):
    """Create detailed popup with point information for all sub-clusters."""
    content = []
...
def main():
    ...
    if not all_stops:
        print("Warning: No stops detected across all files. Adjust noise threshold or check data quality.")
        print(f"NOISE_THRESHOLD: {NOISE_THRESHOLD}m, MIN_SAMPLES: {MIN_SAMPLES}")
        return
    ...
    for label, stops in clusters.items():
        cluster_data.append({
            ...
            'total_points': sum(s.get('points', 1) for s in stops)
        })
    ...
    if map_full_route:
        ...
        for filepath in trip_files:
            ...
            try:
                ...
                # Process traces for this file - no time-based segmentation
                for _, group in trace_df.groupby('id'):
                    points = group[['lat', 'lon']].to_dict('records')
