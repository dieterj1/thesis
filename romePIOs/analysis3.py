...
def create_popup_content(cluster_data, amenities):
    """Create detailed popup with timing and point information."""
    content = []
    
    # Time information
    time_format = "%Y-%m-%d %H:%M:%S"
    first_seen = cluster_data['first_seen'].strftime(time_format)
    last_seen = cluster_data['last_seen'].strftime(time_format)
    total_duration = str(timedelta(seconds=int(cluster_data['total_duration'])))
    
    content.extend([
        f"<b>First Seen:</b> {first_seen}",
        f"<b>Last Seen:</b> {last_seen}",
        f"<b>Total Duration:</b> {total_duration}",
        f"<b>GPS Points:</b> {cluster_data['total_points']}",
        "<hr>"
    ])
...
def main():
    ...
    for label, stops in clusters.items():
        cluster_data.append({
            'lat': np.mean([s['lat'] for s in stops]),
            'lon': np.mean([s['lon'] for s in stops]),
            'first_seen': min(s['start'] for s in stops),
            'last_seen': max(s['end'] for s in stops),
            'total_duration': (max(s['end'] for s in stops) - min(s['start'] for s in stops)).total_seconds(),
            'total_points': sum(s['points'] for s in stops)
        })
    
    # Create map
    map_rome = folium.Map(location=[41.9028, 12.4964], zoom_start=12)
    for cluster in tqdm(cluster_data, desc="Adding markers"):
        amenities = get_amenity_details(cluster['lat'], cluster['lon'])
        
        if not amenities:
            continue
            
        popup_content = create_popup_content(cluster, amenities)
    ...
    map_rome.save('taxi_stopsv3.html')
    print("Map saved to taxi_stopsv3.html")
